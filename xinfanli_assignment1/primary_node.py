    ...
    with ThreadPoolExecutor(max_workers=min(32, len(nodes_sorted))) as ex:
        ...
        for future in as_completed(future_to_task):
            ...
            try:
                ...
                print(
                    f"[primary_node] Node {node['node_id']} failed "
                    f"for slice {sl}: {e}"
                )
                failed_tasks.append((node, sl, str(e)))
    if failed_tasks and successful_nodes:
        print(
            f"[primary_node] Reassigning {len(failed_tasks)} failed task(s)..."
        )

        for failed_node, failed_slice, error in failed_tasks:
            retry_node = successful_nodes[0]

            try:
                print(
                    f"[primary_node] Reassigning slice {failed_slice} "
                    f"from {failed_node['node_id']} "
                    f"to {retry_node['node_id']}"
                )

                retry_result = call_node(retry_node, failed_slice)
                per_node_results.append(retry_result)

            except Exception as retry_error:
                print(
                    f"[primary_node] Retry failed for slice "
                    f"{failed_slice}: {retry_error}"
                )
                raise RuntimeError(
                    f"Failed to complete slice {failed_slice} "
                    f"after worker failure"
                ) from retry_error

    elif failed_tasks:
        raise RuntimeError(
            "All available secondary nodes failed; "
            "cannot reassign failed work."
        )
