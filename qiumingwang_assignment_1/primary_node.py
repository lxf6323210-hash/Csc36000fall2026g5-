...
def distributed_compute(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Determine slice count based on initial active nodes, but DO NOT bind them yet.
    initial_node_count = len(nodes)
    slices = split_into_slices(low, high, initial_node_count)
    ...
    # --- FAULT TOLERANT ---
    def robust_call_node(sl: Tuple[int, int]) -> Dict[str, Any]:
        max_retries = 3
        for attempt in range(max_retries):
            active_nodes = REGISTRY.active_nodes()
            if not active_nodes:
                raise RuntimeError("Cluster Failure: All secondary nodes crashed during execution.")
            
            # Dynamically pick a random healthy node for this slice execution (library imported)
            target_node = random.choice(active_nodes)
            
            try:
                return call_node(target_node, sl)
            except Exception as e:
                print(f"[Fault Tolerance] Node {target_node['node_id']} failed on slice {sl} (Attempt {attempt + 1}): {e}")
                # Evict the dead node immediately so other threads bypass it
                with REGISTRY.lock:
                    REGISTRY.nodes.pop(target_node["node_id"], None)
                    
        raise RuntimeError(f"Task failed permanently: Slice {sl} could not be computed after {max_retries} attempts.")


    with ThreadPoolExecutor(max_workers=min(32, initial_node_count)) as ex:
        futs = [ex.submit(robust_call_node, sl) for sl in slices]
